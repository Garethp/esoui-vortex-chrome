...
const build = async (name, manifestFile = "manifest.json") => {
    ...
    await copyFilesPromise(['build/*', manifestFile, `build-${name}`])

    if (manifestFile !== "manifest.json") {
        fs.renameSync(`build-${name}/${manifestFile}`, `build-${name}/manifest.json`);
    }
    ...
    fs.copyFileSync(`build-${name}/out/${fileName}`, `web-ext-artifacts/${fileName.replace(".zip", `-${name}.zip`)}`);
